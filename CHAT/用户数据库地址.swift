...
class 用户数据库地址 {
    var 用户引用地址 = Database.database().reference().child("users")
    
    func 用户总览(withId uid: String, completion: @escaping (用户Model) -> Void) {
        用户引用地址.child("profile").child(uid).observeSingleEvent(of: DataEventType.value, with: {
            快照 in
    ...
    func 当前用户总览(completion: @escaping (用户Model) -> Void) {
        guard let 当前用户=Auth.auth().currentUser else{
            ...
            用户引用地址.child("profile").child(当前用户.uid).observeSingleEvent(of: DataEventType.value, with: {
                快照 in
                if let 快照值 = 快照.value as? [String:Any]{
                    let 单个用户 = 用户Model.用户转换值(字典: 快照值)
                    completion(单个用户)
    ...
    var 当前用户引用地址 : DatabaseReference? {
        ...
        return 用户引用地址.child("profile").child(当前用户.uid)
    }
}
